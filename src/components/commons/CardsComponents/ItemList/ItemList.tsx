      ...
      return (
        <div className={s.itemListContainer}>
          ...
          <div className={s.controllerContainer}>
            {userId && <button className={s.likeBtn} onClick={() => estado?.includes(props.item.dz_Id)?dislikeSong(props.item.dz_Id, user?.email) :likeSong(props.item.id, user?.email) }><p onClick={()=>{setButtonLike(!buttonLike) }} className={estado?.includes(props.item.dz_Id) | buttonLike ? s.likeImgInclude:s.likeImg}     >♥</p></button>}
            <div>
